...
export default function Home() {
  ...
  const calculateResult = () => {
    ...
    if (calculatedResult !== "Error") {
      ...
      if (currentMode === 'business' && (expression.toLowerCase().includes('loan') || expression.toLowerCase().includes('emi'))) {
        getAiSuggestion('loan');
      }
      if (currentMode === 'business' && expression.toLowerCase().includes('discount')) {
  ...
  const handleAiSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const assistantMessage: AIMessage = { role: "assistant", content: response.answer };
      setAiMessages((prev) => [...prev, assistantMessage]);
    } catch (error) {
      const errorMessage: AIMessage = { role: "assistant", content: "Sorry, I couldn't process that. Please try again." };
  ...
  const getAiSuggestion = async (calculationType: string) => {
    try {
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && ("SpeechRecognition" in window || "webkitSpeechRecognition" in window)) {
        ...
        recognition.onerror = (event: any) => {
            ...
            toast({ variant: 'destructive', title: "Voice Error", description: "Couldn't recognize speech."});
            setIsRecording(false);
        };
        recognition.onend = () => {
            setIsRecording(false);
  ...
  const renderKeypad = (mode: string) => {
    const buttons: (string | { label: string; value: string; className?: string })[] = [];
    
    const actionBtnClass = "bg-secondary hover:bg-secondary/80 text-primary";
    const equalsBtnClass = "bg-accent hover:bg-accent/80 text-accent-foreground row-span-2";

    switch (mode) {
      case "scientific":
        buttons.push(
          {label: 'sin', value: 'sin('}, {label: 'cos', value: 'cos('}, {label: 'tan', value: 'tan('}, {label: 'C', value: 'C', className: actionBtnClass},
          {label: 'log', value: 'log('}, {label: 'ln', value: 'ln('}, {label: '(', value: '('}, {label: ')', value: ')'},
          {label: '√', value: 'sqrt('}, {label: 'x²', value: '^2'}, {label: 'xʸ', value: '^'}, {label: 'π', value: 'Math.PI'},
          ...
          '0', '.', {label: '⌫', value: 'backspace'}, {label: '+', value: '+', className: actionBtnClass},
          {label: '=', value: '=', className: equalsBtnClass + " !col-start-4 !row-start-5 !row-span-2"}
        );
        break;
      case "business":
        buttons.push(
            ...
            '4', '5', '6', {label: '-', value: '-', className: actionBtnClass},
            '1', '2', '3', {label: '+', value: '+', className: actionBtnClass},
            '0', '.', '%', {label: '=', value: '=', className: equalsBtnClass}
        );
        break;
      case "standard":
      default:
        buttons.push(
            {label: 'C', value: 'C', className: actionBtnClass}, {label: '%', value: '%', className: actionBtnClass}, {label: '⌫', value: 'backspace', className: actionBtnClass}, {label: '÷', value: '/', className: actionBtnClass},
            '7', '8', '9', {label: '×', value: '*', className: actionBtnClass},
            '4', '5', '6', {label: '-', value: '-', className: actionBtnClass},
            '1', '2', '3', {label: '+', value: '+', className: actionBtnClass},
            {label: '0', value: '0', className: 'col-span-2'}, '.', {label: '=', value: '=', className: equalsBtnClass}
        );
        break;
    }

    const gridClass = mode === 'scientific' ? 'grid-cols-4 grid-rows-7' : 'grid-cols-4 grid-rows-5';
    ...
    return (
      <div className={`grid ${gridClass} gap-2 sm:gap-3 p-4`}>
        {buttons.map((btn, i) => {
          ...
          return <CalcButton key={i} onClick={action} className={className}>{label}</CalcButton>
        })}
      </div>
    );
  };
  ...
  const CalcButton = ({children, onClick, className = ''}: {children: React.ReactNode, onClick: () => void, className?: string}) => (
    <Button variant="ghost" className={`h-16 sm:h-20 text-2xl sm:text-3xl rounded-2xl transition-all duration-200 ease-in-out active:scale-95 focus:ring-2 ring-accent calc-button-neumorphism active:calc-button-neumorphism-inset ${className}`} onClick={onClick}>
        {children}
    </Button>
  );
      ...
      <main className="w-full max-w-md mx-auto bg-secondary/30 dark:bg-secondary/50 rounded-3xl shadow-2xl dark:shadow-black/50 overflow-hidden border border-border">
              ...
              <SheetContent>
                ...
                <ScrollArea className="h-[calc(100%-4rem)] mt-4">
                  {history.length > 0 ? history.map((item, index) => (
                    <div key={index} className="p-2 mb-2 rounded-lg hover:bg-secondary transition-colors cursor-pointer" onClick={() => handleHistoryClick(item)}>
                      ...
                      <div className="flex justify-between items-center mt-1">
                        ...
                        <Button variant="ghost" size="icon" className="h-6 w-6" onClick={(e) => { e.stopPropagation(); handleCopy(item.result); }}><Copy className="h-4 w-4" /></Button>
                      </div>
                    </div>
                  )) : <p className="text-center text-muted-foreground mt-8">No history yet.</p>}
                </ScrollArea>
              </SheetContent>
            </Sheet>
            <ThemeToggle />
        ...
        <div className="px-4 pb-4">
          <div className="bg-background rounded-2xl p-4 min-h-[120px] text-right flex flex-col justify-end shadow-inner dark:shadow-black/30">
            <p className="text-muted-foreground text-2xl h-8 truncate animate-fade-in">{expression || " "}</p>
            <p className="text-foreground font-bold text-5xl h-14 animate-slide-in-bottom">{result || "0"}</p>
             {aiSuggestion && (
              <div className="text-left mt-2 animate-fade-in">
                  <Button variant="outline" size="sm" className="border-accent text-accent" onClick={() => { setExpression(aiSuggestion); setAiSuggestion('');}}>
        ...
        <Tabs value={currentMode} onValueChange={setCurrentMode} className="w-full">
          <TabsList className="grid w-full grid-cols-5 bg-secondary/80 dark:bg-secondary/70 mx-auto rounded-none h-14">
            ...
            <TabsTrigger value="scientific" className="rounded-none">Scientific</TabsTrigger>
            <TabsTrigger value="business" className="rounded-none">Business</TabsTrigger>
            <TabsTrigger value="statistics" className="rounded-none">Statistics</TabsTrigger>
            <TabsTrigger value="ai" className="rounded-none text-accent">AI</TabsTrigger>
          </TabsList>
          
          <div className="animate-fade-in">
            <TabsContent value="standard">{renderKeypad("standard")}</TabsContent>
            <TabsContent value="scientific">{renderKeypad("scientific")}</TabsContent>
            <TabsContent value="business">{renderKeypad("business")}</TabsContent>
            <TabsContent value="statistics">
              <div className="p-4 text-center">
                  <p className="text-muted-foreground mb-4">Ask AI for statistical calculations!</p>
                  <Button onClick={() => setCurrentMode('ai')}>Switch to AI Mode</Button>
              </div>
            </TabsContent>
            <TabsContent value="ai">
              <div className="p-4 flex flex-col h-[380px] sm:h-[460px]">
                    ...
                    <div className="space-y-4">
                        {aiMessages.map((msg, i) => (
                            <div key={i} className={`flex items-end gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                <div className={`rounded-2xl p-3 max-w-xs sm:max-w-sm ${msg.role === 'user' ? 'bg-primary text-primary-foreground rounded-br-none' : 'bg-secondary text-secondary-foreground rounded-bl-none'}`}>
                                    <p className="text-sm">{msg.content}</p>
                                </div>
                            </div>
                        ))}
                         {isTyping && <div className="flex items-end gap-2 justify-start">
                ...
                <form onSubmit={handleAiSubmit} className="flex items-center gap-2">
                  ...
                  />
                   <Button type="button" size="icon" variant={isRecording ? "destructive" : "outline"} onClick={toggleRecording}>
                        <Mic className="h-5 w-5" />
                    </Button>
                  <Button type="submit" size="icon" className="bg-accent hover:bg-accent/80" disabled={isTyping}>
